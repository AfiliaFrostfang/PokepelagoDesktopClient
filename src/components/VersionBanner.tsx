...
// Cross-deploy notice for the 0.6.2 beta-testing window. Detects beta vs prod at runtime
...
//   - prod  → "v0.6.2 is ready to test on the beta site →" (links to the beta site)
//   - beta  → "you're on the v0.6.2 beta — download the .apworld →" (links to the GitHub Release)
// Dismissible; the dismissal is remembered per-version in localStorage.

const RELEASE_URL = 'https://github.com/dowlle/PokepelagoClient/releases/tag/v0.6.2';
const BETA_URL = 'https://dowlle.github.io/PokepelagoClient/beta/';
const DISMISS_KEY = 'pokepelago_banner_v0_6_2';
    ...
    return (
        ...
        >
            <span className="text-center">
                {isBeta ? (
                    <>
                        You&apos;re testing the <strong>v0.6.2 beta</strong>.{' '}
                        <a href={RELEASE_URL} target="_blank" rel="noreferrer" className="underline font-semibold hover:opacity-80">
                            Download the v0.6.2 .apworld &rarr;
                        </a>
                    </>
                ) : (
                    <>
                        <strong>Poképelago v0.6.2</strong> is ready to test on the{' '}
